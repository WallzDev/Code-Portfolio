...
{
    [Title("Change Material Color By Index")]
    [Description("Changes over time the Color property of an instantiated material of a Renderer component chosen by Index")]

    [Image(typeof(IconColor), ColorTheme.Type.Yellow)]

    [Category("Renderer/Change Material Color By Index")]
    [Parameter("Index", "Index of changed material in the Renderer (counting from 0)")]
    ...
    [Keywords("Set", "Shader", "Hue", "Index")]
    [Serializable]
    public class InstructionRendererChangeMaterialColorByIndex : TInstructionRenderer
    {
        // MEMBERS: -------------------------------------------------------------------------------
        ...
        protected override async Task Run(Args args)
        {
            ...
            materials[m_MaterialIndex] = new Material(materials[m_MaterialIndex]);
            renderer.materials = materials;

            Material materialInstance = materials[m_MaterialIndex];

            string property = this.m_Property.Get(args);
            int propertyID = Shader.PropertyToID(property);
            Color valueSource = materialInstance.GetColor(propertyID);
            ...
            ITweenInput tween = new TweenInput<Color>(
                ...
                (a, b, t) => materialInstance.SetColor(propertyID, Color.Lerp(a, b, t)),
